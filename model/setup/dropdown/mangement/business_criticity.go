...
func init() {
	pkgmodel.Register[*BusinessCriticity]()
}

// BusinessCriticity 业务关键等级
type BusinessCriticity struct {
	pkgmodel.Base
...
func (*BusinessCriticity) GetTableName() string {
	return fmt.Sprintf("%s_%s", model.PREFIX_DROPDOWN, pkgmodel.GetTableName[*BusinessCriticity]())
}
